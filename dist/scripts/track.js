            ...
            for (const key in state) {
                ...
                if (key === "tree") {
                    ...
                    if (Array.isArray(state[key])) {
                        throw new TypeError("cradova err invalid tree element type, should be a single element or parent element from cradova");
                    }
                    if (!(state[key] instanceof HTMLElement)) {
                        console.error("cradova err wrong element type: can't update element state on " +
                            state[key]);
...
export function dispatch(stateID, state) {
    ...
    if (typeof state === "undefined" && typeof stateID === "object") {
        ...
        if (typeof stateID === "string") {
            ...
            if (state === null || state === void 0 ? void 0 : state.cradovaDispatchtrackBreak) {
                updated.push(...nodes);
            }
            else {
