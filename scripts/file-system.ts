 ...
 */
fs.getFileHandle = async function (filePicker: any): Promise<FileSystemFileHandle> {
 ...
 */
fs.writeFile = async function (fileHandle: any, contents: string) {
 ...
 */
fs.verifyPermission = async function (fileHandle: any, withWrite: boolean): Promise<boolean> {
  const opts: any = {};
  if (withWrite) {
    opts.writable = true;
    // For Chrome 86 and later...
 ...
 */
fs.getFileLegacy = (filePicker: { onchange: (e: any) => void; files: any[]; }): Promise<File> => {
...
fs.saveAsLegacy = (filename: string, contents: string) => {
  ...
  document.body.append(aDownloadFile)
